...
def test_version():
    """Test if program returns correct version of the package"""
...
def test_init_command_with_flask():
    with patch(
        ...
        assert "Initializing flask project named new_project" in result.stdout
        assert "Creating project directory" in result.stdout

        mock_init_dir.assert_called_once_with(Path.cwd(), "new_project")
...
def test_init_command_with_fastapi():
    with patch(
        ...
        assert result.exit_code == 0
        assert "Provide a project name" in result.stdout
        assert "Initializing fastapi project named new_project" in result.stdout
        assert "Creating project directory" in result.stdout

        # Check that the _init_project_directory was called with the correct arguments
        mock_init_dir.assert_called_once_with(Path.cwd(), "new_project")
